...
from models import db, Usuario, RolUsuario, Movimiento, TipoMovimiento
...
def aprobar_solicitud(suscripcion_id):
    ...
    try:
        ...
        # 🔥 FIX: Crear pago del MES ACTUAL como APROBADO y luego el próximo mes
        hoy = date.today()
        
        # El comprobante inicial ya fue verificado, por eso se está aprobando la solicitud
        # Esto permite que el paciente use sus consultas gratuitas inmediatamente
        ultimo_dia_actual = monthrange(hoy.year, hoy.month)[1]
        fecha_venc_actual = date(hoy.year, hoy.month, min(10, ultimo_dia_actual))
        
        pago_mes_actual = PagoMensualPrepaga(
            suscripcion_id=suscripcion.id,
            mes=hoy.month,
            anio=hoy.year,
            monto=plan.precio_mensual,
            fecha_vencimiento=fecha_venc_actual,
            estado=EstadoPagoMensual.APROBADO,  # Ya está aprobado con el comprobante inicial
            fecha_aprobacion=datetime.utcnow(),
            aprobado_por=session['user_id'],
            observaciones="Pago inicial aprobado junto con la solicitud de suscripción"
        )
        db.session.add(pago_mes_actual)
        
        # Crear pago mensual para el próximo mes (pendiente)
        if hoy.month == 12:
            proximo_mes = 1
...
def aprobar_pago_mensual(pago_id):
    ...
    try:
        ...
        # 🆕 NUEVO: Resetear contador de consultas para el nuevo mes
        pago.suscripcion.consultas_consumidas = 0
        # Las consultas_disponibles ya están configuradas desde la aprobación inicial
        ...
        # 🔥 SI IMPLEMENTASTE LA SOLUCIÓN DEL PRIMER PROBLEMA (pagos en movimientos)
        # También debes agregar esto:
        from models import Movimiento, TipoMovimiento
        
        movimiento_ingreso = Movimiento(
            tipo=TipoMovimiento.INGRESO,
            monto=pago.monto,
            concepto=f"Pago Prepaga - {pago.suscripcion.usuario.nombre} {pago.suscripcion.usuario.apellido}",
            descripcion=f"Plan: {pago.suscripcion.plan.nombre} - Periodo: {pago.periodo_texto} - DNI: {pago.suscripcion.usuario.dni}",
            pago_prepaga_id=pago.id,
