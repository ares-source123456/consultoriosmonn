...
class SuscripcionPrepaga(db.Model):
    ...
    # 🆕 NUEVOS MÉTODOS PARA VERIFICAR PAGO MENSUAL
    
    def tiene_pago_mes_actual_aprobado(self):
        """
        Verifica si el usuario tiene el pago del mes actual aprobado
        
        Returns:
            bool: True si tiene el pago del mes actual aprobado, False caso contrario
        """
        hoy = date.today()
        ...
        return pago_mes_actual is not None
    
    def puede_usar_consultas(self):
        """
        Verifica si el usuario puede usar las consultas gratuitas de la prepaga
        
        Condiciones que debe cumplir:
        1. Tener suscripción ACTIVA
        2. Tener consultas disponibles (no haberlas agotado)
        3. Tener el pago del mes actual APROBADO
        
        Returns:
            bool: True si puede usar consultas gratuitas, False si debe pagar
        """
        # 1. Verificar que la suscripción esté activa
        if self.estado != EstadoSuscripcion.ACTIVA:
            return False
        
        # 2. Verificar que tenga consultas disponibles
        if not self.tiene_consultas_disponibles():
            return False
        
        # 3. Verificar que tenga el pago del mes actual aprobado
        if not self.tiene_pago_mes_actual_aprobado():
            return False
