...
def calcular_costo_grupo_familiar(usuario_id):
    ...
    return app.config['COSTO_BASE_CONSULTA']
...
def simular_costo():
    ...
    costo_base = app.config['COSTO_BASE_CONSULTA']
    # Cambia esta línea o simplemente pon el descuento en 0
    descuento_total = 0 
    
    costo_final = costo_base # Al ser el descuento 0, el costo final es el base
    ahorro = 0
...
def nuevo_turno():
    if request.method == 'POST':
        try:
            ...
            # 🔥 CAMBIO IMPORTANTE: Usar puede_usar_consultas() en lugar de tiene_consultas_disponibles()
            if suscripcion and suscripcion.puede_usar_consultas():
                # ✅ TIENE PREPAGA CON CONSULTAS DISPONIBLES Y PAGO AL DÍA
                ...
                # ❌ NO TIENE PREPAGA O NO PUEDE USARLA
                
                # Calcular costo
                especialidad_obj = Especialidad.query.get(especialidad_id)
                costo = especialidad_obj.costo_consulta
                ...
                if suscripcion:
                    if not suscripcion.tiene_consultas_disponibles():
                        flash('Se agotaron sus consultas de prepaga. Debe abonar esta consulta.', 'warning')
                    elif not suscripcion.tiene_pago_mes_actual_aprobado():
                        flash('Debe abonar el mes actual de su prepaga para usar consultas gratuitas.', 'warning')
                    else:
            ...
            db.session.rollback()
            flash(f'Error en los datos: {str(ve)}', 'danger')
            return redirect(url_for('nuevo_turno'))
...
def buscar_paciente():
    """
    Búsqueda de pacientes - Soporta vista HTML normal y respuestas AJAX para el dashboard
    """
    # Detectar si es petición AJAX (desde el dashboard)
    is_ajax = request.args.get('ajax') == '1'
    
    # Obtener DNI desde POST (formulario) o GET (AJAX)
    ...
    if dni:
        ...
        if not usuario:
            ...
            return render_template('buscar_paciente.html', usuario=None, turnos=[])
        
        # Obtener todos los turnos donde el usuario es el titular
        turnos = Turno.query.filter_by(paciente_id=usuario.id).all()
        
        # Obtener turnos de familiares del usuario
        familiares = GrupoFamiliar.query.filter_by(usuario_id=usuario.id).all()
        for familiar in familiares:
            turnos_familiar = Turno.query.filter_by(familiar_id=familiar.id).all()
            turnos.extend(turnos_familiar)
        
        # Ordenar por fecha y hora (más recientes primero)
        turnos.sort(key=lambda x: (x.fecha, x.hora), reverse=True)
        
        # Si es AJAX, devolver datos en JSON
        if is_ajax:
            return construir_respuesta_ajax(usuario, turnos)
        
        # Vista HTML normal - limitar a últimos 10 turnos
        turnos_limitados = turnos[:10]
        return render_template('buscar_paciente.html',
                             usuario=usuario,
                             turnos=turnos_limitados)
    
    # Sin DNI - mostrar formulario vacío
    return render_template('buscar_paciente.html', usuario=None, turnos=[])


def construir_respuesta_ajax(usuario, turnos):
    """
    Construye la respuesta JSON para peticiones AJAX desde el dashboard
    """
    hoy = date.today()
    turnos_data = []
    
    for turno in turnos:
        # Determinar si es turno de hoy o futuro
        es_futuro = turno.fecha > hoy
        
        # Determinar datos del paciente (titular o familiar)
        if turno.familiar_id and turno.familiar:
            paciente_nombre = f"{turno.familiar.nombre} {turno.familiar.apellido}"
            dni_paciente = turno.familiar.dni
        else:
            paciente_nombre = f"{turno.paciente.nombre} {turno.paciente.apellido}"
            dni_paciente = turno.paciente.dni
        
        # Información del especialista
        especialista_nombre = None
        if turno.especialista:
            especialista_nombre = f"Dr/a. {turno.especialista.apellido}"
        
        # Información del pago
        pago_estado = None
        pago_id = None
        puede_marcar_abonado = False
        
        if turno.pago:
            pago_estado = turno.pago.estado.value
            pago_id = turno.pago.id
            
            # Puede marcar como abonado si está pendiente y no tiene comprobante
            puede_marcar_abonado = (
                turno.pago.estado == EstadoPago.PENDIENTE and
                not turno.pago.comprobante
            )
        
        # Construir diccionario del turno
        turno_dict = {
            'id': turno.id,
            ...
            'pago_id': pago_id,
            'puede_marcar_abonado': puede_marcar_abonado,
            'es_futuro': es_futuro
        }
...
@app.cli.command()
def suspender_suscripciones_vencidas():
    """
    Suspende suscripciones que no tienen pago del mes anterior aprobado
    Ejecutar este comando mensualmente (ej: con cron)
    """
    ...
    for suscripcion in suscripciones_activas:
        # Verificar si tiene pago del mes anterior
        pago_mes_anterior = PagoMensualPrepaga.query.filter_by(
            suscripcion_id=suscripcion.id,
        ...
        if not pago_mes_anterior:
            # No pagó el mes anterior, suspender
